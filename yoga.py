...
@app.route('/yoga_poses', methods=['GET'])
def get_yoga_poses():
    return {'yoga_poses': yoga_poses}
    


# @app.route('/yoga_poses/<string:english_name>', methods=['GET'])
# def get_yoga_poses(pose_name):
#    for pose in yoga_poses:
#         if pose['english_name'] == pose_name:
#             return {'yoga_pose': pose}
#         return 'pose mot found', 404   

@app.route('/yoga_poses/<int:pose_id>', methods=['GET'])
def get_yoga_pose(pose_id):
    for pose in yoga_poses:
        if pose['id'] == pose_id:
            return {'yoga_pose': pose}
    return 'Pose not found', 404

@app.route('/yoga_poses', methods=['POST'])
def add_yoga_pose():
    pose = {
        'id': yoga_poses[-1]['id'] + 1,
        'name': request.json['name'],
        'difficulty': request.json['difficulty'],
        'description': request.json['description']
    }
    yoga_poses.append(pose)
    return {'yoga_pose': pose}, 201
